...
const navItems = [
  { label: 'Dashboard', href: '/reporter/dashboard', icon: LayoutDashboard },
  { label: 'My Articles', href: '/reporter/articles', icon: FileText },
  { label: 'Write Article', href: '/reporter/articles/new', icon: FilePlus },
  { label: 'Media Library', href: '/reporter/media', icon: Image },
  { label: 'My Profile', href: '/reporter/profile', icon: User },
];
  ...
  return (
    <>
      ...
      <aside
        ...
        aria-label="Reporter navigation"
      >
        <div className="flex items-center justify-between h-16 px-4 border-b border-white/10">
          <div className="flex items-center gap-3">
            ...
            {sidebarOpen && <span className="hidden md:block font-serif font-bold text-lg truncate">Reporter</span>}
          </div>
          <Button
            variant="ghost"
            size="icon"
        ...
        <nav className="flex-1 py-4 space-y-1 px-2">
          {navItems.map((item) => {
            const isActive =
              pathname === item.href ||
              (item.href !== '/reporter/articles' && pathname.startsWith(`${item.href}/`));
            return (
              <Link
                key={item.href}
                href={item.href}
